...
# 2+、根据规则修改类名
function rename_prefix_in_classContent {
# 2.1、类添加前缀和后缀
for (( i = 0; i < ${#config_content_array[@]}; i++ )); do
original_class_name=${config_content_array[i]};
temp_class_name=${original_class_name:2};
result_class_name="${class_prefix}${temp_class_name}"
